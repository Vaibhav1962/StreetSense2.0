...
def test_search(q, viewbox=None, bounded=0):
...
# Vaibhav's likely location near Vaishali/Kaushambi: Lat ~28.64, Lon ~77.33
# Bounding box around Kaushambi/Vaishali (offset 0.1)
lon = 77.33
lat = 28.64
offset = 0.1
vb = f"{lon-offset},{lat+offset},{lon+offset},{lat-offset}"

test_search("Yashoda Hospital Kaushambi")
test_search("Max Hospital Vaishali")
test_search("[amenity=hospital]", viewbox=vb, bounded=1)
test_search("hospital", viewbox=vb, bounded=1)
test_search("healthcare", viewbox=vb, bounded=1)
test_search("[healthcare=hospital]", viewbox=vb, bounded=1)
